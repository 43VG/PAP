...
app = criar_app()  # Cria e configura a aplicação Flask usando a função definida em __init__.py
...
if __name__ == "__main__":  # Verifica se o ficheiro está a ser executado diretamente (e não importado por outro)
    app.run(debug=True)  # Inicia o servidor Flask em modo de depuração 

with app.app_context():  # Abre novamente o contexto da app
    print(Utilizador.query.all())  # Mostra no terminal todos os utilizadores registados na base de dados (útil para debug)
